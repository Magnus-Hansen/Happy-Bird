...
class NeuralNetwork{
  ...
  NeuralNetwork(int input, int hidden, int output){
    ...
    IHWeights = random(hiddenNodes, inputNodes);
    HOWeights = random(outputNodes, hiddenNodes);
    Hbias = random(hiddenNodes, 1);
    Obias = random(outputNodes, 1);
  }
  ...
  float[] feedForward(float[] inputArray){
    ...
    hidden = Product(IHWeights, input);
    hidden.add(Hbias);
    ...
    output = Product(HOWeights, hidden);
    output.add(Obias);
  ...
  void train(float[] inputArray, float[] targetArray){
    ...
    Matrix targets = FromArray(targetArray);
    Matrix outputErrors = subtract(targets, output);
    ...
    Matrix DHOWeights = Product(gradient, hiddenT);

    HOWeights.add(DHOWeights);

    Obias.add(gradient);

    Matrix HOWeightsT = transpose(HOWeights);
    Matrix hiddenErrors = Product(HOWeightsT, outputErrors);
    ...
    Matrix DIHWeights = Product(hiddenGradient, inputT);
